let GraphDraw = function(graph){
    ...
    let default_point_radius_func = graph.nodes.length < 125 ? () => 9/current_zoom : (graph.nodes.length < 1000 ? () => 7/current_zoom : () => 5/current_zoom )
    ...
    return {
        ...
        setNodeRadius : function( func ){
            current_point_radius_func = function(d){
                ...
                if( v < 0 ) v = default_point_radius_func(d)
                return v
            }
            node.attr( "r", current_point_radius_func )
        },
        resetNodeRadius : function(){
            current_point_radius_func = default_point_radius_func
            node.attr( "r", current_point_radius_func )
        },
